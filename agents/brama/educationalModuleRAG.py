...
voyage_api_key = os.environ.get('VOYAGE_API_KEY')  # Optional - only needed for educational mode
...
def educational_mode():
    ...
    # Voyage API key is optional - educational mode will work without it (with reduced functionality)
    if not voyage_api_key:
        print("Warning: VOYAGE_API_KEY not set. Educational mode requires Voyage API key.")
        print("Educational mode is not available without VOYAGE_API_KEY.")
        return
    ...
    try:
        ...
        print(f"Error initializing educational mode: {e}")
        return
